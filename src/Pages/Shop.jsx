...
const Shop = () => {
  ...
  const filteredAndSortedProducts = () => {
    ...
    return sortedProducts.filter((product) => {
      const matchesRating =
        ...
        activeFilters.rating.includes(Math.round(product.rating?.rate || 0));
      ...
      const matchesDiscount =
        ...
        activeFilters.discount.some((disc) => {
          const num = parseInt(disc); // "10% Off" → 10
          return product.discount >= num;
        });

      return matchesRating && matchesCategory && matchesColor && matchesDiscount;
    });
  };
  ...
  const handleToggleWishlist = (product) => {
    ...
    setAlertMessages({
      ...alertMessages,
      [product.id]: {
        msg: isAdded ? "Item removed from wishlist!" : "Item added to wishlist!",
        type: "wishlist",
      },
    });
    setTimeout(() => {
      ...
      <div className="flex flex-col lg:flex-row p-8 min-h-screen gap-6">
        ...
        <div className="lg:w-3/4">
          ...
          <div className="bg-white p-4 rounded-lg shadow-md mb-6">
            <h2 className="text-xl font-bold mb-4 text-[#1D3178]">
                ...
                <div className={`${viewType === "list" ? "flex-grow p-4 flex flex-col" : "p-4"}`}>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      ...
                      onClick={() => setZoomedProduct(product.id)}
                    >
                      <FaSearchPlus />
                    </button>
                  </div>
          ...
          <div className="fixed top-0 left-1/2 transform -translate-x-1/2 z-50 space-y-2">
            {Object.keys(alertMessages).map((productId) => {
              ...
              return (
                <div
                  key={productId}
                  className={`bg-${alert.type === "cart" ? "pink" : "pink"}-500 text-white p-4 rounded-md shadow-lg transition-opacity duration-1000`}
                >
                  {alert.msg}
                </div>
              );
